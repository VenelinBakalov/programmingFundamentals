...
namespace Legendary_Farming
{
    class Program
    {
            ...
            while (true)
            {
                ...
                for (int i = 1; i < materialsObtained.Length; i += 2)
                {
                    ...
                    if (materials[materialsObtained[i]] >= 250)
                    {
                        ...
                        {
                            if (pair.Value >= 250 && pair.Key.Equals("shards"))
                            {
                                legendarySword = "Shadowmourne";
                                isEnough = true;
                            }
                            else if (pair.Value >= 250 && pair.Key.Equals("fragments"))
                            {
                                legendarySword = "Valanyr";
                                isEnough = true;
                            }
                            else if (pair.Value >= 250 && pair.Key.Equals("motes"))
                            {
                                legendarySword = "Dragonwrath";
                                isEnough = true;
                            }
                        }
                        //   if (materials[materialsObtained[i]].Equals("shards"))
                        //   {
                        //       legendarySword = "Shadowmourne";
                        //       isEnough = true;
                        //       break;
                        //   }
                        //   else if (materials[materialsObtained[i]].Equals("fragments"))
                        //   {
                        //       legendarySword = "Valanyr";
                        //       isEnough = true;
                        //       break;
                        //   }
                        //   else if (materials[materialsObtained[i]].Equals("motes"))
                        //   {
                        //       legendarySword = "Dragonwrath";
                        //       isEnough = true;
                        //       break;
        ...
        {
            foreach (KeyValuePair<string, int> material in legendaryMaterials.OrderByDescending(x => x.Value).ThenBy(x => x.Key))
            {
                Console.WriteLine($"{material.Key}: {material.Value}");
            }
        }

        private static void FillDictionaries(Dictionary<string, int> materials, Dictionary<string, int> legendaryMaterials, SortedDictionary<string, int> otherMaterials)
        {
