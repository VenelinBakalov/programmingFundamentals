...
namespace User_Logs
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] commandLine = Console.ReadLine().Split(splitters, StringSplitOptions.RemoveEmptyEntries).Where(entry => !entry.Equals("IP") && !entry.Equals("message") && !entry.Equals("user")).ToArray();
            ...
            while (!commandLine[0].Equals("end"))
            {
                ...
                commandLine = Console.ReadLine().Split(splitters, StringSplitOptions.RemoveEmptyEntries).Where(entry => !entry.Equals("IP") && !entry.Equals("message") && !entry.Equals("user")).ToArray();
            }
        ...
        private static void InsertIps(SortedDictionary<string, Dictionary<string, int>> userLogs, string userName, string ip)
        {
